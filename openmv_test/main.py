...
while True:
	...
	if audio_playing and time.ticks_diff(current_time, last_audio_time) > AUDIO_DURATION:
		audio_playing = False
	img = sensor.snapshot()
	...
	if time.ticks_diff(current_time, last_report_time) > OBSTACLE_REPORT_INTERVAL:
			if obstacle_detected:
				...
				if not audio_playing:
					...
					pending_obstacle = center_x
			else:
				data = f"N,0,0,{int(current_angle)},E\n"
				if pending_obstacle is not None and not audio_playing:
					obstacle_point(pending_obstacle)
					audio_playing = True
					last_audio_time = current_time
					pending_obstacle = None
